...
;;     ;; (turn-on-haskell-indent) is not supported...
;;
;;     (offside-trap-mode)  ;; overrides some of haskell-indentation keys
;;     ))
...
(defun offside-trap-find-sticky-lines ()
  ...
  ;; if the first line is empty, no search
...
(defun offside-trap-set-sticky-lines ()
  (let* ((sticky-end (offside-trap-find-sticky-lines))
    ...
    (offside-trap-overlay-sticky-lines (offside-trap-bol (line-number-at-pos)) (offside-trap-eol offside-trap-sticky-line-end))
    ))
...
(defun offside-trap-block-indent-step (&optional first)
  (interactive)
  ;; Now haskell-mode's indent-for-tab-command checks last-command
  ;; to see whether it is a repeated call or not. We must fake last-command.
  (if (not first) (setq last-command 'indent-for-tab-command))
...
(defun offside-trap-block-indent ()
  ...
  (offside-trap-block-indent-step t)
...
(defvar offside-trap-mode-map
  (let ((keymap (make-sparse-keymap)))
    ...
    ;; (define-key keymap (kbd "<C-return>") 'haskell-indentation-newline-and-indent)
