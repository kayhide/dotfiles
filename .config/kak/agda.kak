...
hook global WinSetOption filetype=agda %{
    ...
    hook window ModeChange pop:insert:.* -group agda-trim-indent  agda-trim-indent
    hook window InsertChar \n -group agda-insert agda-insert-on-new-line
...
add-highlighter shared/agda/macro        region ^\K#                            (?<!\\)\n        fill meta
...
add-highlighter shared/agda/quasiquote   region \[\b[_a-z]['\w]*#?\| \|\]                        regex \[\b[_a-z]['\w]*#?\|(.*?)\|\] 1:string

add-highlighter shared/agda/code/ regex (?<!')\b0x+[A-Fa-f0-9]+ 0:value
add-highlighter shared/agda/code/ regex (?<!')\b\d+([.]\d+)? 0:value
add-highlighter shared/agda/code/ regex (?<!')\b(import|hiding|qualified|module)(?!')\b 0:keyword
add-highlighter shared/agda/code/ regex (?<!')\b(import)(?!')\b[^\n]+(?<!')\b(as)(?!')\b 2:keyword
add-highlighter shared/agda/code/ regex (?<!')\b(class|data|default|deriving|infix|infixl|infixr|instance|module|newtype|pattern|type|where)(?!')\b 0:keyword
add-highlighter shared/agda/code/ regex (?<!')\b(case|do|else|if|in|let|mdo|of|proc|rec|then)(?!')\b 0:attribute
add-highlighter shared/agda/code/ regex (?<!')\b(type|data)\b\s+(\bfamily\b)?(?!') 0:keyword
...
add-highlighter shared/agda/code/ regex ^\s*(?:where\s+|let\s+|default\s+)?([_a-z]['\w]*#?(?:,\s*[_a-z]['\w]*#?)*)\s+::\s 1:meta

# matches deriving strategies
add-highlighter shared/agda/code/ regex \bderiving\s+\b(stock|newtype|anyclass|via)\b 1:keyword
add-highlighter shared/agda/code/ regex \bderiving\b\s+(?:[A-Z]['\w]+|\([',\w\s]+?\))\s+\b(via)\b 1:keyword
...
define-command -hidden agda-insert-on-new-line %{
